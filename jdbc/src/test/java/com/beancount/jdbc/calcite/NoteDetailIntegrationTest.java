...
final class NoteDetailIntegrationTest {

    @Test
    void noteDetailReturnsRows() throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:calcite:", props);
                ...
                ResultSet rs =
                        statement.executeQuery(
                                "SELECT \"id\", \"account\", \"comment\" "
                                        + "FROM \"beancount\".\"note_detail\" "
                                        + "ORDER BY \"id\" FETCH FIRST 5 ROWS ONLY")) {
            int rowCount = 0;
            while (rs.next()) {
                rowCount++;
            }
            assertTrue(rowCount > 0, "Expected note_detail to return rows");
        }
    }
}

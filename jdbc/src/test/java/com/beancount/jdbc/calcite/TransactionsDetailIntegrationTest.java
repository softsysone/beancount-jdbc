...
final class TransactionsDetailIntegrationTest {

    @Test
    void transactionsDetailTableReturnsRows() throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:calcite:", props);
                ...
                ResultSet rs =
                        statement.executeQuery(
                                "SELECT \"id\", \"flag\", \"payee\" FROM \"beancount\".\"transactions_detail\" ORDER BY \"id\" FETCH FIRST 5 ROWS ONLY")) {
            int rowCount = 0;
            while (rs.next()) {
                rowCount++;
            }
            assertTrue(rowCount > 0, "Expected transactions_detail to return rows");
        }
    }
}
